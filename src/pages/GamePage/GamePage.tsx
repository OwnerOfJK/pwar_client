...
import { usePwarProvider } from "@/providers/PwarProvider"
...
const GamePage: React.FC = () => {
    ...
    const { handlePwarAction, selectedUnit } = usePwarProvider()
    ...
    useEffect(() => {
        ...
        const handleCellClick = async (cell?: Coordinate) => {
            if (!dialogContainerRef || !dialogContainerRef.current || !cell) return

            // If there's a selected unit, handle Pwar interaction
            if (selectedUnit) {
                try {
                    await handlePwarAction({
                        type: "INTERACT",
                        params: {
                            position: cell,
                            color: color,  // Using the color from PixelawProvider
                            for_player: 0, // Default values, adjust as needed
                            for_system: 0
                        }
                    })
                } catch (error) {
            ...
            // Otherwise handle normal PixelAW interaction
    ...
    }, [pixelawCore, selectedUnit, handlePwarAction, color])
